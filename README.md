...
# Shuffle the values
You can change the argumnets values, but **remember** - the lower time to die is and higher time to eat or sleep, the bigger chances some philosoper **will not survive**
Moreover, If the number of philosophers is an odd number and the time to die is too short or sleeping/eating time to high, the higher chances one of philosoper will start starving and die. Using even numbers, they will swap forks swiftly.

# Fun fact
Number of philosophers equals number of forks. Forks' number needed to eat is 2. If you set the philo number to 1, there is only 1 fork, meaning that philosopher will die after 800ms (or the time you set yourself)
